...
import { getPalette, palettes, paperColors, renderArtwork, stageAt } from "@/lib/artwork";
import { applyAlgorithm, changeAppearance, changeRecipe, checkGeometry, finishAdjustment, getServerSnapshot, getSnapshot, playPause, previewAppearance, previewRecipe, redo, report, reroll, seek, setSpeed, stepConstruction, subscribe, undo } from "@/lib/studio-store";
...
function IconButton({ label, children, onClick, disabled = false, active = false }: { label: string; children: ReactNode; onClick: () => void; disabled?: boolean; active?: boolean }) {
  return <Tooltip><TooltipTrigger asChild><Button variant={active ? "secondary" : "ghost"} size="icon" aria-label={label} disabled={disabled} onClick={onClick}>{children}</Button></TooltipTrigger><TooltipContent>{label}</TooltipContent></Tooltip>;
}
...
const controlGroups: { name: string; controls: { key: NumericRecipeKey; label: string; description: string; min: number; max: number }[] }[] = [
  ...
  { name: "Paths & connections", controls: [
    ...
    { key: "branchReach", label: "Branch reach", description: "Sets how far side branches can extend.", min: 1, max: 10 },
...
export default function Home() {
  ...
  return <TooltipProvider delayDuration={350}><main className="studio">
    ...
    <div className="workspace"><aside className="control-panel">
      ...
      <TabsContent value="finish"><section className="control-section first-section"><span className="field-label">Color studies</span><div className="palette-list">{palettes.map(item => <button key={item.id} aria-pressed={appearance.palette === item.id} className={`palette-button ${appearance.palette === item.id ? "selected" : ""}`} onClick={() => changeAppearance({ palette: item.id })}><span className="swatches">{Object.entries(item.primary).map(([family, color]) => <i key={family} style={{ background: color }}/>)}</span>{item.name}</button>)}</div><p className="field-note compact">{palette.note}</p>{(recipe.orientation === "both" || recipe.algorithm === "bridge") && <div className="second-palette"><span className="swatches">{Object.entries(palette.secondary).map(([family, color]) => <i key={family} style={{ background: color }}/>)}</span><span>{recipe.algorithm === "bridge" ? "Second colored segment" : "Second color region"}</span></div>}</section>
      ...
      <section className="control-section"><div className="switch-row"><label htmlFor="live-shading">Live shading</label><Switch id="live-shading" checked={appearance.live} onCheckedChange={live => changeAppearance({ live })}/></div><p className="field-note compact">Show shading as solid structures form. Turn it off to watch the linework first.</p></section>
      </TabsContent></Tabs>
      ...
      <ArtworkView svg={renderArtwork(state.scene, state.drawing, appearance, progress)} paper={paperColors[appearance.paper]}/>
      <div className="canvas-bottom"><span>{state.scene.strokes.filter(stroke => stroke.kind === "seed").length} seeds · {state.scene.strokes.length} strokes · {state.drawing.faces.length} surface groups</span><span>{recipe.orientation === "both" ? "TWO ORIENTATIONS" : recipe.orientation === "below" ? "ALTERNATE ORIENTATION" : "PRIMARY ORIENTATION"}</span></div>
